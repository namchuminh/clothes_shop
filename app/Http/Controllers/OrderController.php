...
class OrderController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $carts = Cart::with('detail_products.product', 'detail_products.size', 'color', 'topping')
            ->where('customer_id', $customer_id)
            ->get();

        // Tính tổng giá trị dựa trên price của detail_products, color và topping
        $totalPrice = $carts->sum(function ($cart) {
            $productPrice = $cart->detail_products->price * $cart->quantity;
            $colorPrice = $cart->color ? $cart->color->price : 0;
            $toppingPrice = $cart->topping ? $cart->topping->price : 0;
            return $productPrice + $colorPrice + $toppingPrice;
        });
