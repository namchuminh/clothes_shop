...
@section('title-breadcrumb', 'DANH SÁCH SẢN PHẨM')
@section('content')
<style>
    .image{
        text-align: center;
                ...
                <div class="col-xl-4 col-lg-6">
                    <div class="product-image-items">
                        <div class="product-image">
                            <img style="width: 100%; height: 500px;" class="image_origin" src="" alt="img" class="w-100">
                        </div>
                    </div>
                </div>
                ...
                <div class="col-xl-3 col-lg-4">
                    <div class="product-form-wrapper">
                        <form action="#" id="contact-forms" method="POST">
                            ...
                            <div class="form-clt">
                                <label class="select-crust">Chọn Màu Sắc</label>
                                <div class="nice-select" tabindex="0">
                                    <span class="current">
                                        Chọn Màu Sắc
                                    </span>
                                    <ul class="list color-list" style="min-width: fit-content;">

                                    </ul>
                                </div>
                            </div>
                            <div class="form-clt">
                                <label class="select-crust">Chọn Phụ Kiện</label>
                                <div class="nice-select" tabindex="0">
                                    <span class="current">
                                        Chọn Phụ Kiện
                                    </span>
                            ...
                            <div class="form-clt">
                                <button type="submit" class="theme-btn bg-dark add-to-cart">
...
<script>
    $(document).ready(function() {
        ...
        function fetchDataBorder() {
            $.ajax({
                url: `{{ $api_url }}products/${id}/color`,
                method: 'GET',
                success: function(response) {
                    response.forEach(item => {
                        $(".color-product").append(`<li>${item.color.name}</li>`);
                        $(".color-list").append(`
                            <li data-value="${item.color.id}" value="${item.color.price}" class="option">
                                ${item.color.name} (+ ${Number(item.color.price).toLocaleString('vi-VN')}Đ)
                            </li>
                        `);
                    });

                    $('.color-list .option').on('click', function() {
                        colorPrice = Number($(this).attr('value'));
                        $(".price").text(((priceDisplay + toppingPrice + colorPrice) * qtyValue).toLocaleString('vi-VN') + "đ");
                    });
                },
                error: function(xhr) {
                    console.log(xhr)
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    $('.size-list .option').on('click', function() {
                        ...
                        priceDisplay = value;
                        $(".price").text(Number((priceDisplay + toppingPrice + colorPrice) * qtyValue).toLocaleString('vi-VN') + "đ");
                    });
                },
                error: function(xhr) {
                    console.log(xhr)
        ...
        $('.add-to-cart').on('click', function(event) {
            ...
            var color = $('.color-list .selected').data('value');
